...
class RestaurantTableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        {
            navigationController?.navigationBar.largeTitleTextAttributes = [NSAttributedString.Key.foregroundColor: UIColor(red: 231/255, green: 67/255, blue: 60/255, alpha: 1), NSAttributedString.Key.font: customFont]
        }
...
//        navigationController?.hidesBarsOnSwipe = true
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.thumbnaiImageView.image = UIImage(named: restaurants[indexPath.row].image)
        //解決畫面一滾動，cell重複利用後會重複打勾
...
//        let messageAC = UIAlertController(title: "server Unavailable", message: "Sorry, the call number is error.", preferredStyle: .alert)
//        messageAC.addAction(okAction)
//        let callAction = UIAlertAction(title: "Call \(indexPath.row)", style: .default) { (UIAlertAction) in
//            
//           self.present(messageAC, animated: true, completion: nil)
//            
//        }
...
//        }else
//        {
//            checkTitle = "Undo Check In"
//        }
//        let checkInAction = UIAlertAction(title: checkTitle, style: .default) { (UIAlertAction) in
//            
////            let cell = tableView.cellForRow(at: indexPath)
//            let cellVC = tableView.cellForRow(at: indexPath) as! RestaurantTableViewCell
//           
//            if self.restaurantIsVisted[indexPath.row]
//            {
//                self.restaurantIsVisted[indexPath.row] = false
////                cell?.accessoryType = .none
//                cellVC.checkInImageView.image = nil
//                
//            }else
//            {
...
//               
//                cellVC.checkInImageView.image = UIImage(named: "heart-tick")
//                
//                
//            }
//
...
//
    override func tableView(_ tableView: UITableView, leadingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        var checkTitle: String?
            ...
            {
                checkTitle = "Undo CheckIn"
            }else
            {
                checkTitle = "Check In"
            }
        
        let visitedAction = UIContextualAction(style: .normal, title: checkTitle) { (action, sourveView, completionHandler) in
            ...
            }else
           {
            ...
            cell.checkInImageView.isHidden = self.restaurants[indexPath.row].isVisited ? false : true
            completionHandler(true)
        }
        visitedAction.backgroundColor = UIColor(red: 23, green: 44, blue: 89)
        visitedAction.image = UIImage(named: "tick")
        let swipConfigurgation = UISwipeActionsConfiguration(actions: [visitedAction])
        return swipConfigurgation
    }
    override func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        ...
        let shareAction = UIContextualAction(style: .normal, title: "Share") { (action, sourceView, completionHandler) in
            let activeController : UIActivityViewController
            let defaultText = "Just check in at \(self.restaurants[indexPath.row].name)"
            
            if let imageToShare = UIImage(named: self.restaurants[indexPath.row].image)
            {
            activeController = UIActivityViewController(activityItems: [defaultText,imageToShare], applicationActivities: nil)
                
            }else
            {
                activeController = UIActivityViewController(activityItems: [defaultText], applicationActivities: nil)
                
            }
            
            if let popoverController = activeController.popoverPresentationController{
                if let cell = tableView.cellForRow(at: indexPath)
                {
                popoverController.sourceView = cell
                popoverController.sourceRect = cell.bounds
        ...
        delectAction.backgroundColor = .gray
        shareAction.backgroundColor = .red
        delectAction.image = UIImage(named: "delete")
        shareAction.image = UIImage(named: "share")
        let swipConfiguration = UISwipeActionsConfiguration(actions: [delectAction, shareAction])
